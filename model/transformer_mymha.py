...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, supp_mask):
        # q: [(h*b), Nq, c]
        # k: [(h*b), Nk, c]
        # v: [(h*b), Nv, c]
        # supp_mask: [(h*b), Nk]
        fg_mask = (~supp_mask).unsqueeze(-2).float() 
        attn = torch.bmm(q, k.transpose(1, 2)) # [(h*b), Nq, Nk] [(h*b), 14, 600]
        attn = attn + fg_mask * -10000.0
...
class MultiHeadAttentionOne(nn.Module):
    ...
    def forward(self, q, k, v, supp_mask, query_input=False):
        # input q: BxNxC
        # input k,v: BxCxHxW or BxCxN
        # supp_mask: [6,600]
        ...
        supp_mask = supp_mask.repeat(n_head, 1)

        output, attn, log_attn = self.attention(q, k, v, supp_mask)
