...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = AAformer(layers=args.layers, shot=args.shot, \
    ...
    if args.weight:
        if os.path.isfile(args.weight):
            ...
            # for name, param in model.named_parameters(): #仅仅只包含可训练的部分,把加载那部分参数固定住
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch>args.epochs//5:  #40个epoch之后才会每个都验证并保存
...
def train(train_loader, model, optimizer, transformer_optimizer, epoch, base_lrs):
    ...
    for i, (input, target, s_input, s_mask, s_init_seed, subcls) in enumerate(train_loader):
        ...
        if args.base_lr > 1e-6:
            poly_learning_rate(optimizer, base_lrs, current_iter, max_iter, power=args.power, index_split=index_split,
                               warmup=args.warmup, warmup_step=len(train_loader))
